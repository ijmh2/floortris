...
`createCustomFurniture` creates one measured object only in an active layout proposal. It accepts a bounded label; a closed solid floor-furniture kind (`sofa`, `chair`, `table`, `coffee_table`, `desk`, `storage`, `bed` or `plant`); exact width, depth, height and top-left position in centimetres; a quarter-turn rotation; a local furniture palette; and optionally a verified `linkedDeskId` for a chair. It is revision-bound and idempotent. The placement is checked against the same complete engine and is refused atomically if it introduces a blocking issue. Rugs and other non-solid floor layers are deliberately out of scope for custom creation.
...
A custom `sofa` can opt into declarative sectional geometry without allowing arbitrary polygons or models. Pass `geometry: { type: "sectional", primaryFacing, modules }`; each of 2–12 modules has a safe local ID, closed `seat`/`corner`/`chaise` type, exact local `xCm`, `yCm`, `widthCm`, `depthCm`, `heightCm`, and cardinal `facing`. Modules must form one positive-length edge-sharing connected assembly, cannot overlap positive area, and must exactly fill the declared measured envelope bounds. This supports genuine L and U unions while keeping one immutable furniture object. Collision, custom-floor boundary, height, fixture and route occupancy use the module union, so an empty L/U notch stays empty. `primaryFacing` alone controls TV and coffee-table alignment; every module seating front is checked after internal joined spans are subtracted. Parent movement and quarter-turn rotation transform the entire union. Internal module IDs are never independent tool targets.

Sectionals render with joined module cushions and only exposed backs/arms. The board and inspector say **CUSTOM SECTIONAL** and disclose every module measurement. Generic polygons, curves, recliner mechanics, arbitrary upholstery meshes, module editing and remote assets remain deliberately unsupported; remove and recreate the one-off object to change its topology.
