...
export const TOOL_SCHEMAS: Record<string, { title: string; description: string; inputSchema: Schema; readOnly: boolean }> = {
  ...
  listCatalogue: { title: 'Browse the catalogue', readOnly: true, description: 'List named available size variants and valid palette IDs. Optional profile/tag filters only narrow the list.', inputSchema: object({ kind, profile: profileKind, tag: str, offset: integer(), limit: integer(1, 50) }) },
  ...
  setRoomGeometry: { title: 'Set room size', readOnly: false, description: 'Stage rectangular room dimensions or name in a setup proposal. Human Confirm room inputs is required. Existing fixed fixtures remain present.', inputSchema: object({ ...mutation, widthCm: num(240, 1000), depthCm: num(240, 1000), name: str }, mutationRequired) },
  setOpening: { title: 'Set a door or window', readOnly: false, description: 'Stage a complete door or window record in a setup proposal. Accepted openingLocks are human-only pins and cannot be overridden. Hinged doors use an open 90 degree pose; unsupported mechanisms are explicitly blocked.', inputSchema: object({ ...mutation, opening: openingSchema }, [...mutationRequired, 'opening']) },
