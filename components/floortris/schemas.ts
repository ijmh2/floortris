...
export const TOOL_SCHEMAS: Record<string, { description: string; inputSchema: Schema; readOnly: boolean }> = {
  ...
  listCatalogue: { readOnly: true, description: 'List named available size variants and valid palette IDs. These are demo designs, not real purchasable SKUs.', inputSchema: object({ kind, offset: integer(), limit: integer(1, 50) }) },
  ...
  setOpening: { readOnly: false, description: 'Stage a complete door or window record in a setup proposal. Hinged doors use an open 90 degree pose; unsupported mechanisms are explicitly blocked.', inputSchema: object({ ...mutation, opening: openingSchema }, [...mutationRequired, 'opening']) },
  ...
  proposeLayout: { readOnly: false, description: 'Run a bounded deterministic greedy planner inside an existing layout proposal. Preserves owned instances and locks; adds required lounge functions before optional desk/table/rug. Results can be partial. Not an external AI or proof that no layout exists. Cancellation or intervening edits prevent commit.', inputSchema: object({ ...mutation, variantIds: { type: 'array', items: str, maxItems: 8 } }, mutationRequired) },
