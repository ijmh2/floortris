...
export function registerFloortrisTools(store: FloortrisStore, onStatus: (status: WebMCPState) => void, hostDocument: Document = document, onToolResult?: (result: CommandResult) => void) {
  ...
  if (!native || typeof native.registerTool !== 'function') { onStatus({ state: 'unsupported', count: 0, message: 'This browser does not expose document.modelContext, so no tools could be registered. WebMCP is still behind a flag: open this page in ChatGPT\u2019s in-app browser, or in Google Chrome with chrome://flags/#enable-webmcp-testing set to Enabled and relaunched. All human editing and local planning work regardless.' }); return () => { disposed = true; controller.abort(); }; }
