...
import { decodeShareFragment, encodeShareFragment } from './share-links.ts';
...
function Board({ layout, room, rules, inventory, report, title, revision, compact, selected, onSelect, onEdit, onPin, onNotice, mode, editable, stamp, suggestions, onAccept, draggedVariant, onDropVariant, focusCells, onEditRoom }: {
  ...
  const [ghost,setGhost]=useState<Furniture|null>(null),[draggingId,setDraggingId]=useState<string|null>(null),[dockPoint,setDockPoint]=useState<{x:number;y:number}|null>(null);
  ...
  const down=(e:React.PointerEvent<HTMLButtonElement>,p:Furniture,resize=false)=>{
    ...
    drag.current={piece:p,startX:e.clientX,startY:e.clientY,cellX:board.current.clientWidth/room.widthCm,cellY:board.current.clientHeight/room.depthCm,stamp,resize};setDraggingId(p.id);
  };
  ...
  const up=(e:React.PointerEvent<HTMLButtonElement>)=>{
    const d=drag.current,p=dragPiece(e.clientX,e.clientY);drag.current=null;setGhost(null);setDraggingId(null);
  ...
  return <section className={`ft-board-shell ${compact?'ft-compact':''}`} aria-label={`${title} board`} style={{'--aspect':room.widthCm/room.depthCm} as React.CSSProperties}>
    ...
    <div className="ft-board-wrap"><div className="ft-ruler-side">{fmt(room.depthCm/100)} m</div>
      <div ref={board} className={`ft-board ft-mode-${mode} ${room.floorPlan ? 'ft-custom-floorplan' : ''}`} aria-label={`${title} grid`} style={{aspectRatio:`${room.widthCm}/${room.depthCm}`,clipPath:planClipPath(room),'--grid-x':`${20/room.widthCm*100}%`,'--grid-y':`${20/room.depthCm*100}%`,'--floor':floorFinish?.color,'--wall':PALETTES.wall.find(p=>p.id===layout.appearance.wall)?.color,...(mode==='furniture'&&floorFinish?.texture?{backgroundImage:`url("${floorFinish.texture.url}")`,backgroundSize:`${floorFinish.texture.repeatCm[0]/room.widthCm*100}% ${floorFinish.texture.repeatCm[1]/room.depthCm*100}%`}:{})} as React.CSSProperties}
        ...
        {layout.furniture.map(p=>{const shown=ghost?.id===p.id?ghost:p,isDragging=draggingId===p.id,moduleText=p.geometry?`, custom sectional with ${p.geometry.modules.length} measured modules`:'';return <button key={p.id} className={`${p.kind==='tv'?'ft-wall-tv':'ft-furniture'} ${p.kind==='rug'?'ft-rug':''} ${p.ownership==='custom'?'ft-custom-piece':''} ${p.geometry?'ft-custom-sectional':''} ${p.wallAnchor?`ft-wall-${p.wallAnchor.wall}`:''} ${selected===p.id?'selected':''} ${p.locked.position?'ft-locked':''} ${isDragging?'ft-is-dragging':''}`} style={position(displayBounds(shown))} title={`${p.geometry?'CUSTOM SECTIONAL':p.ownership==='custom'?'CUSTOM':''} · ${shown.label} · ${shown.sizeCm.w} × ${shown.sizeCm.d} × ${shown.sizeCm.h??'?'} cm${p.locked.position?' · pinned':''}`} aria-label={`${p.geometry?'Custom sectional':p.ownership==='custom'?'Custom measured furniture':'Furniture'}, ${shown.label}${moduleText}, ${shown.sizeCm.w} by ${shown.sizeCm.d} by ${shown.sizeCm.h??'unknown height'} centimetres${p.locked.position?', position locked':''}`}
          onPointerDown={e=>down(e,p)} onPointerMove={e=>{if(drag.current)setGhost(dragPiece(e.clientX,e.clientY));}} onPointerUp={up} onPointerCancel={()=>{drag.current=null;setGhost(null);setDraggingId(null);}} onKeyDown={e=>keyDown(e,p)} onFocus={()=>{if(editable)onSelect(p.id);}} onClick={()=>onSelect(p.id)}>
          {p.kind==='tv'?<span>TV · {shown.sizeCm.w} cm</span>:<>{shown.geometry?<SectionalShape item={shown}/>:<Shape item={shown}/>}<span className="ft-item-label">{shown.geometry?'CUSTOM SECTIONAL ':shown.ownership==='owned'?'YOUR ':shown.ownership==='custom'?'CUSTOM ':''}{pieceLabel(shown)}</span><span className="ft-item-dim">{shown.sizeCm.w} × {shown.sizeCm.d} cm</span></>}{p.locked.position && <span className="ft-pin-badge" title="Pinned">▣</span>}
        </button>;})}
...
function FloortrisWorkspace({ store: suppliedStore }: { store?: FloortrisStore }) {
  ...
  const [review, setReview] = useState<{ id: string; revision: number; conflicts?: { key: string; label: string; yours: { id: string; label: string } | null; proposal: { id: string; label: string } | null }[]; choices?: Record<string, 'yours' | 'proposal'> } | null>(null);
  ...
  useEffect(() => {
    ...
    if (result.operationSucceeded && result.generatedRoom) { setSelected(null); setPanel(null); setReview(null); setShowSetup(false); setFocusCells([]); setMode('furniture'); setNotice({ text: 'Proposal ready — compare when you’re ready.', error: false }); }
  ...
  const editable = view !== 'compare' && !(which === 'proposal' && active?.kind !== 'layout');
  ...
  useEffect(()=>{
    if(shareProcessed.current||!window.location.hash.startsWith('#share='))return;shareProcessed.current=true;
    void decodeShareFragment(window.location.hash).then(imported=>{
      if(!imported){setNotice({text:'That share link is invalid, damaged or above the safe size limit. Nothing changed.',error:true});return;}
      enterRoom(imported);const url=new URL(window.location.href);url.hash='';window.history.replaceState(null,'',url);setNotice({text:`Opened shared room ${imported.room.name} as a separate local room. Nothing was uploaded.`,error:false});
    });
  // eslint-disable-next-line react-hooks/exhaustive-deps -- a share fragment is consumed exactly once on mount.
  },[]);
  ...
  const canEditSnapshot = (w: 'current' | 'proposal', base: string) => {
    ...
    if (editStamp(s,w) !== base) { setNotice({text:'That branch changed during this gesture. Try again on its latest revision.',error:true}); return false; }
    return true;
  };
  ...
  const find = async (id: string) => {
    if (!active || active.kind !== 'layout') return;
  ...
  const applyReview = () => {
    if (!review) return;
    const result = store.applyProposal(review.id, review.revision, review.choices);
    if (result.error?.code === 'reconciliation_required') {
      setReview({ ...review, conflicts: (result.conflicts as NonNullable<typeof review>['conflicts']) || [] });
      setNotice({ text: 'Choose which version to keep for each overlapping change.', error: false });
      return;
    }
    onResult(result); if (result.operationSucceeded) { setReview(null); changeView('current'); }
  };
  ...
  const downloadText=(name:string,text:string,type:string)=>{const url=URL.createObjectURL(new Blob([text],{type})),link=document.createElement('a');link.href=url;link.download=name;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
  const shareRoom=async()=>{
    const encoded=await encodeShareFragment(state);if(!encoded.fragment){setNotice({text:encoded.reason||'Use JSON export for this room.',error:true});exportRoom();return;}
    const url=new URL(window.location.href);url.hash=encoded.fragment;
    try{await navigator.clipboard.writeText(url.toString());setNotice({text:`Share link copied. ${encoded.compressed?'Compressed ':''}room data stays inside the URL; nothing was uploaded.`,error:false});}
    catch{downloadText('floortris-share-link.txt',url.toString(),'text/plain');setNotice({text:'Clipboard access was unavailable, so the server-free share link was downloaded as text.',error:false});}
  };
  const exportScheduleCsv=()=>{const schedule=buildFurnitureSchedule(state,which);downloadText('floortris-furniture-schedule.csv',`\ufeff${scheduleToCsv(schedule)}`,'text/csv;charset=utf-8');};
  const openPrintableSchedule=()=>{const schedule=buildFurnitureSchedule(state,which),url=URL.createObjectURL(new Blob([scheduleToPrintableHtml(schedule)],{type:'text/html'}));const opened=window.open(url,'_blank','noopener,noreferrer');if(!opened)downloadText('floortris-move-in-schedule.html',scheduleToPrintableHtml(schedule),'text/html');setTimeout(()=>URL.revokeObjectURL(url),60_000);};
  ...
  return <div className="ft-app ft-overhaul">
    ...
    <main className="ft-stage" inert={modalOpen}>
      ...
      <section className="ft-canvas-panel" aria-label="Room planning workspace">
        ...
        <div className={`ft-proposal-row ft-status-${status}`} aria-live="polite"><div className="ft-proposal-message"><strong>{active ? active.kind === 'setup' ? 'Room inputs · awaiting confirmation' : `Proposal · ${status === 'ready_for_review' ? 'ready' : status}` : 'Your room. Another possibility.'}</strong><span>{blocking ? `${repairItem?.label || 'Layout'} · ${issueName(blocking)}` : active ? `Revision ${active.revision} · Yours stays unchanged until Apply` : 'Let the planner arrange pieces around your locks.'}</span></div><div className="ft-proposal-actions">{busy ? <button className="ft-button ft-secondary" onClick={() => abort.current?.abort()}>Cancel search</button> : <button className="ft-button ft-secondary" onClick={plan} disabled={active?.kind === 'setup'} title={active ? 'Re-run the planner over this independent draft.' : 'Run the bounded local planner on a new draft. Yours stays unchanged.'}>{active ? 'Re-plan draft' : 'Try a proposal'}</button>}<button className="ft-button ft-secondary" onClick={()=>void compareStrategies()} disabled={strategyBusy || busy || active?.kind === 'setup'}>{strategyBusy ? 'Comparing…' : 'Compare strategies'}</button>{active && <><button className="ft-button ft-secondary" onClick={() => active.kind === 'setup' ? (setPanel(null), setSelected(null), setShowSetup(true)) : changeView('proposal')}>View</button><button className="ft-button ft-primary" disabled={status !== 'ready_for_review' || active.kind !== 'layout'} onClick={() => setReview({ id: active.id, revision: active.revision })}>Apply</button><button className="ft-button ft-secondary" onClick={discard}>Discard</button></>}{status === 'blocked' && repairItem && <button className="ft-button ft-primary" disabled={busy} onClick={() => find(repairItem.id)}>Find placements</button>}</div></div>
    ...
    {review && <div className="ft-modal-backdrop"><section ref={reviewModal} tabIndex={-1} className="ft-modal" role="dialog" aria-modal="true" aria-labelledby="review-title"><h2 id="review-title">Make this arrangement yours?</h2><p>Human confirmation only. Apply reconciles the proposal with the latest Yours and checks every hard rule again.</p>{review.conflicts?.length ? <div className="ft-reconciliation"><p>This proposal was designed from an earlier arrangement. Choose each overlapping change:</p>{review.conflicts.map(conflict => <div key={conflict.key} className="ft-reconciliation-item"><strong>{conflict.label}</strong><button className={review.choices?.[conflict.key] === 'yours' ? 'active' : ''} onClick={()=>setReview({ ...review, choices: { ...review.choices, [conflict.key]: 'yours' } })}>Keep Yours</button><button className={review.choices?.[conflict.key] === 'proposal' ? 'active' : ''} onClick={()=>setReview({ ...review, choices: { ...review.choices, [conflict.key]: 'proposal' } })}>Keep Proposal</button></div>)}</div> : null}<div className="ft-modal-actions"><button className="ft-button ft-secondary" onClick={()=>{setReview(null);changeView('compare');}}>Compare first</button><button className="ft-button ft-primary" onClick={applyReview}>Apply revision {review.revision}</button></div><button className="ft-text-button" onClick={()=>setReview(null)}>Cancel</button></section></div>}
  </div>;
}
