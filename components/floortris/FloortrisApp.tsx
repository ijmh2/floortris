...
function Board({ layout, room, rules, inventory, report, title, revision, compact, selected, onSelect, onEdit, onPin, onNotice, mode, editable, stamp, suggestions, onAccept, draggedVariant, onDropVariant }: {
  ...
  mode:OverlayMode;editable:boolean;stamp:string;suggestions:Candidate[];onAccept:(c:Candidate)=>void;draggedVariant:string|null;onDropVariant:(piece:Furniture,stamp:string)=>void;
}) {
  ...
  return <section className={`ft-board-shell ${compact?'ft-compact':''}`} aria-label={`${title} board`}>
    ...
    <div className="ft-board-wrap"><div className="ft-ruler-side">{fmt(room.depthCm/100)} m</div>
      <div ref={board} className={`ft-board ft-mode-${mode}`} aria-label={`${title} grid`} style={{aspectRatio:`${room.widthCm}/${room.depthCm}`,'--grid-x':`${20/room.widthCm*100}%`,'--grid-y':`${20/room.depthCm*100}%`,'--floor':PALETTES.floor.find(p=>p.id===layout.appearance.floor)?.color,'--wall':PALETTES.wall.find(p=>p.id===layout.appearance.wall)?.color} as React.CSSProperties}
        ...
        {room.fixtures.map(p=><div key={p.id} className={`ft-fixed ft-fixed-${p.kind}`} style={position(bounds(p))}><span>{p.label.toUpperCase()}</span></div>)}
        {layout.furniture.map(p=>{const shown=ghost?.id===p.id?ghost:p;return <button key={p.id} className={`${p.kind==='tv'?'ft-wall-tv':'ft-furniture'} ${p.kind==='rug'?'ft-rug':''} ${p.wallAnchor?`ft-wall-${p.wallAnchor.wall}`:''} ${selected===p.id?'selected':''} ${p.locked.position?'ft-locked':''}`} style={position(displayBounds(shown))} title={`${p.label} · ${p.sizeCm.w} × ${p.sizeCm.d} × ${p.sizeCm.h??'?'} cm${p.locked.position?' · pinned':''}`} aria-label={`${p.label}, ${p.sizeCm.w} by ${p.sizeCm.d} centimetres${p.locked.position?', position locked':''}`}
          ...
          {p.kind==='tv'?<span>TV · {p.sizeCm.w} cm</span>:<><Shape item={shown}/><span className="ft-item-label">{p.ownership==='owned'?'YOUR ':''}{p.kind==='coffee_table'?'TABLE':p.kind.toUpperCase()}</span><span className="ft-item-dim">{shown.sizeCm.w} × {shown.sizeCm.d} cm</span></>}{p.locked.position && <span className="ft-pin-badge" title="Pinned">▣</span>}
        </button>;})}
        ...
        {previewPiece && <div className={`ft-drag-outline ${preview?.blocking.length?'ft-drag-invalid':''}`} style={position(displayBounds(previewPiece))}><span className="ft-drag-chip">{preview?.blocking[0]?.code || `${previewPiece.label} · snap 20 cm`}{preview?.blocking.length?dockPiece?' · drop rejected':' · drop to keep & repair':''}</span></div>}
...
function FloortrisWorkspace({ store: suppliedStore }: { store?: FloortrisStore }) {
  ...
  const [store] = useState(() => suppliedStore || createStore(typeof window === 'undefined' ? session.makeInitial() : loadState(session)));
  ...
  useEffect(() => { try { localStorage.setItem(session.storageKey, JSON.stringify(state)); } catch { queueMicrotask(() => setNotice({ text: 'Local saving unavailable. Export your room before closing.', error: true })); } }, [state, session]);
  ...
  const changeView = (v: View) => { setView(v); setSelected(null); setPanel(null); setSuggestions([]); };
  ...
  const add = (variantId: string) => { if (!editable) return; const r = store.humanAdd(which, variantId); onResult(r); if (r.operationSucceeded) { select(r.objectId as string); setNotice({ text: 'Added. Drag to place; any conflicts stay visible.', error: false }); } };
  const pin = (id: string) => {
  ...
  const editIn = (w:'current'|'proposal', id:string, patch:PlacementPatch, base:string) => {
    ...
    if (r.operationSucceeded) { const s=store.getState(),p=w==='current'?{layout:s.current,room:s.room,rules:s.rules}:s.proposal!; const issues=validate(p.layout,p.room,p.rules,s.inventory).issues;const blocked=issues.find(i=>i.severity==='block' && i.objectIds.includes(id));if(blocked)setNotice({text:`${blocked.code} · move kept for repair`,error:true}); }
  };
  ...
  const dockProps = (id:string) => ({draggable:editable,onDragStart:(e:React.DragEvent<HTMLButtonElement>)=>{e.dataTransfer.setData('text/plain',id);e.dataTransfer.effectAllowed='copy';setDraggedVariant(id);},onDragEnd:()=>setDraggedVariant(null)});
  const plan = async () => {
  ...
  const find = async (id: string) => {
    ...
    changeView('proposal'); select(id); setBusy(true); abort.current = new AbortController();
  ...
  return <div className="ft-app ft-overhaul">
    <header className="ft-header"><Brand /><h1>{state.room.name}</h1><div className="ft-header-actions"><span className="ft-save-label">Saved on this device</span><button className="ft-button ft-secondary" aria-expanded={panel === 'room'} onClick={() => togglePanel('room')}>Room <span aria-hidden="true">⌄</span></button></div></header>
    <main className="ft-stage">
      <section className="ft-canvas-panel" aria-label="Room planning workspace">
        <div className="ft-board-chrome"><div className="ft-view-tabs" role="group" aria-label="Layout view">{(['current', 'proposal', 'compare'] as View[]).map(v => <button key={v} aria-pressed={view === v} className={view === v ? 'active' : ''} onClick={() => changeView(v)} disabled={v !== 'current' && !active}>{v === 'current' ? 'Yours' : v === 'proposal' ? 'Proposal' : 'Compare'}</button>)}</div><button className={`ft-tools-chip ${webmcp.state === 'registered' ? 'ft-tools-ready' : ''}`} onClick={() => togglePanel('tools')}><span aria-hidden="true">●</span> {webmcp.state === 'registered' ? `${webmcp.count} tools ready` : webmcp.state === 'checking' ? 'Checking tools…' : 'Tools off'}</button></div>
        ...
        <div className="ft-overlay-toolbar"><div role="group" aria-label="Board overlay" className="ft-mode-tabs">{(['furniture','height','walk','tv','doors'] as OverlayMode[]).map(m => <button key={m} aria-pressed={mode === m} onClick={() => { setMode(m); setTvFlash(0); if(m==='tv')setTvBadge(false); }} className={(tvFlash?'tv':mode) === m ? 'active' : ''}>{m === 'tv' ? `TV${tvBadge?' !':''}` : m[0].toUpperCase() + m.slice(1)}</button>)}</div><div className="ft-health-flags">{(['path','tv','door'] as const).map(f => <button key={f} className={`ft-health-${flagState(f)}`} onClick={() => { setMode(f === 'path' ? 'walk' : f === 'door' ? 'doors' : 'tv'); setPanel('check'); setSelected(null); }}><span aria-hidden="true">{flagState(f) === 'clear' ? '✓' : flagState(f) === 'missing' ? '○' : '!'}</span> {f === 'tv' ? 'TV' : f} · {flagState(f)}</button>)}</div></div>
        <div className={`ft-boards ${view === 'compare' ? 'ft-comparison' : ''}`}>
          {(['current','proposal'] as const).filter(w=>w==='current'?(view!=='proposal'||!active):(view!=='current'&&!!active)).map(w=>{
            ...
            return <Board key={w} layout={l} room={r} rules={ru} inventory={state.inventory} report={rep} title={w==='current'?'Yours':'Proposal'} revision={w==='current'?state.currentRevision:active!.revision} compact={view==='compare'} selected={view==='compare'?null:selected} onSelect={select} onEdit={(id,patch,base)=>editIn(w,id,patch,base)} onPin={pin} onNotice={text=>setNotice({text,error:true})} mode={tvFlash?'tv':mode} editable={view!=='compare'&&(w==='current'||(active?.kind==='layout'&&status!=='stale'))} stamp={editStamp(state,w)} suggestions={w==='proposal'?suggestions:[]} onAccept={accept} draggedVariant={draggedVariant} onDropVariant={(p,base)=>dropIn(w,p,base)}/>;
          })}
        </div>
        <div className="ft-board-bottom"><span>20 cm grid · drag to move · R rotate · arrows nudge</span><button className="ft-text-button" onClick={() => togglePanel('check')}>Room check · {report.validation.hardFailures ? `${report.validation.hardFailures} to fix` : report.brief.status === 'incomplete' ? 'brief incomplete' : 'clear'} ↗</button></div>
        <nav className="ft-dock" aria-label="Furniture dock"><button className="ft-dock-open" aria-expanded={panel === 'pieces'} onClick={() => togglePanel('pieces')}><span>＋</span>Pieces</button>{['frame-tv-120','line-desk-100','pebble-table-80','fern-40'].map(id => { const v = CATALOGUE.find(v => v.id === id)!; return <button key={id} className="ft-dock-piece" {...dockProps(id)} disabled={!editable} onClick={() => add(id)} title={`Add ${v.name}`}><span className="ft-dock-shape"><Shape item={{kind:v.kind, appearance:v.palette}} small /></span><span>{v.kind === 'tv' ? 'TV' : v.kind === 'coffee_table' ? 'Table' : v.kind === 'plant' ? 'Plant' : 'Desk'}</span></button>; })}<span className="ft-dock-hint">Drag a piece onto the grid</span></nav>
      </section>
      {(panel || item) && <aside className={`ft-drawer ${panel === 'pieces' ? 'ft-pieces-drawer' : ''}`} aria-label={panel ? `${panel} panel` : 'Selected piece'}><div className="ft-drawer-head"><h2>{panel === 'pieces' ? 'Pieces' : panel === 'room' ? 'Room' : panel === 'tools' ? 'Agent tools' : panel === 'check' ? `${view === 'compare' ? 'Yours' : which === 'proposal' ? 'Proposal' : 'Yours'} · room check` : item?.label}</h2><button className="ft-icon-button" aria-label="Close panel" onClick={() => { setPanel(null); setSelected(null); }}>×</button></div>
        {panel === 'pieces' && <><div className="ft-brief-icons" aria-label="Required room brief">{rules.requiredKinds.map(k => <span key={k}>{layout.furniture.some(f => f.kind === k) ? '✓' : '○'} {k === 'tv' ? 'TV' : nice(k)}</span>)}</div><h3>Yours</h3>{state.inventory.map(o => <button key={o.id} className="ft-owned-card" onClick={() => { if (!layout.furniture.some(f => f.id === o.id)) setView('current'); select(o.id); }}><strong>{o.label}</strong><span>{o.sizeCm.w} × {o.sizeCm.d} cm · measured</span></button>)}<button className="ft-text-button" onClick={() => setShowOwned(!showOwned)}>+ Add measured piece</button>{showOwned && <form onSubmit={e => { e.preventDefault(); const r = store.humanAddOwned({label:ownedForm.label,kind:ownedForm.kind as Furniture['kind'],sizeCm:{w:ownedForm.w,d:ownedForm.d,h:ownedForm.h}}); onResult(r); if(r.operationSucceeded){setView('current'); select(r.objectId as string);setShowOwned(false);} }}><label className="ft-field"><span>Piece name</span><input required maxLength={80} value={ownedForm.label} onChange={e => setOwnedForm({...ownedForm,label:e.target.value})}/></label><label className="ft-field"><span>Type</span><select value={ownedForm.kind} onChange={e=>setOwnedForm({...ownedForm,kind:e.target.value})}>{['sofa','chair','desk','coffee_table','storage','plant','bed','rug'].map(k=><option key={k} value={k}>{nice(k)}</option>)}</select></label><div className="ft-form-grid">{(['w','d','h'] as const).map(k=><NumberField key={k} label={`${k.toUpperCase()} measured`} value={ownedForm[k]} min={1} max={500} step={1} onChange={n=>setOwnedForm({...ownedForm,[k]:n})}/>)}</div><button className="ft-button ft-primary">Add to Yours</button></form>}<h3>Named variants</h3><div className="ft-variant-list">{CATALOGUE.map(v=><button key={v.id} {...dockProps(v.id)} disabled={!editable} onClick={()=>add(v.id)}><span className="ft-dock-shape"><Shape item={{kind:v.kind,appearance:v.palette}} small/></span><span><strong>{v.name}</strong><small>{v.sizeCm.w} × {v.sizeCm.d} cm</small></span><span>+</span></button>)}</div><p className="ft-muted">Demo catalogue. No purchasing or real SKUs.</p></>}
        {panel === 'room' && <><a className="ft-button ft-secondary ft-full" href={session.compact ? '/' : '/?sample=3m'}>{session.compact ? 'Return to original room ↗' : 'Open 3 × 3 m sample ↗'}</a>{session.compact && <p className="ft-muted">Separate sample · your original room is preserved. Assumed door, window and radiator positions. View the furnished proposal before applying.</p>}<button className="ft-button ft-secondary ft-full" onClick={()=>{setPanel(null);setSelected(null);setShowSetup(true);}}>Size, doors & windows ↗</button><p className="ft-muted">{state.room.widthCm} × {state.room.depthCm} cm · demo measurements. Radiator is a fixed demo fixture.</p>{(['wall','floor'] as const).map(target=><div key={target} className="ft-palette-section"><h3>{target === 'wall' ? 'Walls' : 'Floor'} · {which === 'current' ? 'Yours' : 'Proposal'}</h3><div className="ft-swatches">{PALETTES[target].map(p=><button key={p.id} disabled={!editable} title={p.name} aria-label={`Set ${p.name} ${target} finish`} aria-pressed={layout.appearance[target]===p.id} className={layout.appearance[target]===p.id?'active':''} style={{background:p.color}} onClick={()=>onResult(store.humanSetRoomFinish(which,target,p.id))}>{layout.appearance[target]===p.id?'✓':''}</button>)}</div></div>)}<button className="ft-button ft-secondary ft-full" onClick={exportRoom}>Export room JSON</button><button className="ft-text-button" onClick={()=>{if(window.confirm('Reset this room to its starting sample? Export first to keep a copy.')){store.resetDemo(session.makeInitial());changeView('current');}}}>Reset demo</button></>}
        {panel === 'tools' && <><strong>{webmcp.state === 'registered' ? `${webmcp.count} native tools ready` : 'Native tools unavailable'}</strong><p>{webmcp.message}</p><p>Agents edit proposals on this board. Apply, Discard and Unlock stay with you.</p><p>Try a proposal runs a bounded, deterministic local planner—not a chatbot.</p><h3>Board assumptions</h3><p>{rules.walkHardCm} cm hard / {rules.walkPreferredCm} cm preferred walking width. TV checks a full-width height strip at {rules.H_lowCm} cm, not optical visibility.</p><p>No safety or accessibility certification. Your room stays in this browser.</p></>}
        {panel === 'check' && <><p>{report.brief.status === 'satisfied' ? 'Required brief complete.' : `Missing: ${report.brief.missingRequired.map(nice).join(', ')}`}</p>{report.issues.map((issue,i)=><button key={i} className={`ft-issue ft-issue-${issue.severity}`} onClick={()=>{setMode(issue.code.startsWith('tv_')?'tv':issue.code.startsWith('door_')?'doors':'walk');}}><strong>{nice(issue.code)}</strong><p>{issue.message}</p></button>)}{!report.issues.length && <p>✓ No rule issues.</p>}{active?.omitted.map((o,i)=><p key={i}>{o.variantId || o.objectId}: {o.reason}</p>)}<p className="ft-muted">TV flags are a height rule, not proof of real-world visibility.</p></>}
