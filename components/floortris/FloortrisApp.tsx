  ...
  return <div className="ft-app ft-overhaul">
    ...
    <main className="ft-stage">
      <section className="ft-canvas-panel" aria-label="Room planning workspace">
        ...
        <div className="ft-overlay-toolbar">{dimension === '3d' ? <span className="ft-3d-hint">Architectural preview · edit placement & inspect flags in 2D</span> : <div role="group" aria-label="Board overlay" className="ft-mode-tabs">{(['furniture','height','walk','tv','doors'] as OverlayMode[]).map(m => <button key={m} aria-pressed={mode === m} onClick={() => { setMode(m); setTvFlash(0); if(m==='tv')setTvBadge(false); }} className={(tvFlash?'tv':mode) === m ? 'active' : ''}>{m === 'tv' ? `TV${tvBadge?' !':''}` : m[0].toUpperCase() + m.slice(1)}</button>)}</div>}<div className="ft-health-flags">{(['path','tv','door'] as const).map(f => <button key={f} className={`ft-health-${flagState(f)}`} onClick={() => { setDimension('2d'); setMode(f === 'path' ? 'walk' : f === 'door' ? 'doors' : 'tv'); setPanel('check'); setSelected(null); }}><span aria-hidden="true">{flagState(f) === 'clear' ? '✓' : flagState(f) === 'missing' ? '○' : '!'}</span> {f === 'tv' ? 'TV' : f} · {flagState(f)}</button>)}</div></div>
        <div className={`ft-boards ${view === 'compare' ? 'ft-comparison' : ''}`}>
          {(['current','proposal'] as const).filter(w=>w==='current'?(view!=='proposal'||!active):(view!=='current'&&!!active)).map(w=>{
            ...
            if(dimension==='3d') return <ThreeViewBoundary key={w} onReturn={()=>setDimension('2d')}><Suspense fallback={<div className="ft-3d-loading" role="status">Opening your room in 3D…</div>}><Room3D room={r} layout={l} rules={ru} title={w==='current'?'Yours':'Proposal'} revision={w==='current'?state.currentRevision:active!.revision} selected={view==='compare'?null:selected} onSelect={select} onReturn2D={()=>setDimension('2d')} compact={view==='compare'} selectable={view!=='compare'}/></Suspense></ThreeViewBoundary>;
      ...
      {(panel || item) && <aside className={`ft-drawer ${panel === 'pieces' ? 'ft-pieces-drawer' : ''}`} aria-label={panel ? `${panel} panel` : 'Selected piece'}><div className="ft-drawer-head"><h2>{panel === 'pieces' ? 'Pieces' : panel === 'room' ? 'Room' : panel === 'tools' ? 'Agent tools' : panel === 'check' ? `${view === 'compare' ? 'Yours' : which === 'proposal' ? 'Proposal' : 'Yours'} · room check` : item?.label}</h2><button className="ft-icon-button" aria-label="Close panel" onClick={() => { setPanel(null); setSelected(null); }}>×</button></div>
        ...
        {panel === 'check' && <><p>{report.brief.status === 'satisfied' ? 'Required brief complete.' : `Missing: ${report.brief.missingRequired.map(nice).join(', ')}`}</p>{report.issues.map((issue,i)=><button key={i} className={`ft-issue ft-issue-${issue.severity}`} onClick={()=>{setDimension('2d');setMode(issue.code.startsWith('tv_')?'tv':issue.code.startsWith('door_')?'doors':'walk');}}><strong>{nice(issue.code)}</strong><p>{issue.message}</p></button>)}{!report.issues.length && <p>✓ No rule issues.</p>}{active?.omitted.map((o,i)=><p key={i}>{o.variantId || o.objectId}: {o.reason}</p>)}<p className="ft-muted">TV flags are a height rule, not proof of real-world visibility.</p></>}
