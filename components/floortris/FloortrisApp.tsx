  ...
  return <div className="ft-app ft-overhaul">
    ...
    <main className="ft-stage">
      ...
      {(panel || item) && <aside className={`ft-drawer ${panel === 'pieces' ? 'ft-pieces-drawer' : ''}`} aria-label={panel ? `${panel} panel` : 'Selected piece'}><div className="ft-drawer-head"><h2>{panel === 'pieces' ? 'Pieces' : panel === 'room' ? 'Room' : panel === 'tools' ? 'Agent tools' : panel === 'check' ? `${view === 'compare' ? 'Yours' : which === 'proposal' ? 'Proposal' : 'Yours'} · room check` : item?.label}</h2><button className="ft-icon-button" aria-label="Close panel" onClick={() => { setPanel(null); setSelected(null); }}>×</button></div>
        ...
        {panel === 'room' && <><p className="ft-muted">Use Rooms above to return to saved rooms or open samples. Each room keeps its own proposal.</p><button className="ft-button ft-secondary ft-full" onClick={()=>{setPanel(null);setSelected(null);setShowSetup(true);}}>Room inputs & fixed fixtures ↗</button><p className="ft-muted">{room.widthCm} × {room.depthCm} cm · edit your measurements and fixed features. Changes need your confirmation.</p>{(['wall','floor'] as const).map(target=><div key={target} className="ft-palette-section"><h3>{target === 'wall' ? 'Walls' : 'Floor'} · {which === 'current' ? 'Yours' : 'Proposal'}</h3><div className="ft-swatches">{PALETTES[target].map(p=><button key={p.id} disabled={!editable} title={p.name} aria-label={`Set ${p.name} ${target} finish`} aria-pressed={layout.appearance[target]===p.id} className={layout.appearance[target]===p.id?'active':''} style={{background:p.color}} onClick={()=>onResult(store.humanSetRoomFinish(which,target,p.id))}>{layout.appearance[target]===p.id?'✓':''}</button>)}</div></div>)}<button className="ft-button ft-secondary ft-full" onClick={exportRoom}>Export room JSON</button><button className="ft-text-button" onClick={()=>{if(window.confirm('Reset this room to its starting sample? Export first to keep a copy.')){store.resetDemo(session.makeInitial());changeView('current');}}}>Reset demo</button></>}
        {panel === 'tools' && <><strong>{webmcp.state === 'registered' ? `${webmcp.count} native tools ready` : 'Native tools unavailable'}</strong><p>{webmcp.message}</p><h3>Live tool calls</h3>{toolLog.length === 0 ? <p className="ft-muted">Nothing yet. Every WebMCP call an agent makes appears here as it happens — including the ones this page refuses.</p> : <ol className="ft-tool-feed">{toolLog.map(e => <li key={e.seq} className={e.ok ? '' : 'ft-tool-refused'}><span className="ft-tool-line"><code>{e.name}</code>{e.readOnly && <em>read</em>}<b>{e.ok ? [e.revision != null ? `rev ${e.revision}` : null, e.validationStatus ? e.validationStatus : null, e.hardFailures ? `${e.hardFailures} blocking` : null].filter(Boolean).join(' · ') || 'ok' : `refused · ${e.errorCode || 'error'}`}</b></span>{e.args && <span className="ft-tool-args">{e.args}</span>}</li>)}</ol>}<p>Ask an agent to generateRoom with dimensions, room type and openings. It opens a new furnished proposal and saves your previous room in Rooms. Apply, Discard and Unlock stay with you.</p><p>Try a proposal runs a bounded, deterministic local planner—not a chatbot.</p><h3>Board assumptions</h3><p>{rules.walkHardCm} cm hard / {rules.walkPreferredCm} cm preferred walking width. {showTv ? `TV checks a full-width height strip at ${rules.H_lowCm} cm, not optical visibility.` : profile.kind==='bedroom' ? `Beds need a connected ${rules.bedLongSideAccessCm} cm side-entry zone of at least 100 cm, excluding up to 60 cm at the head.` : 'Approach zones connect each required activity to the entrance.'}</p><p>No safety or accessibility certification. Your room stays in this browser.</p></>}
