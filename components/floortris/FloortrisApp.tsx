...
function loadState(session: ReturnType<typeof roomSession>): AppState {
  try { const raw = localStorage.getItem(session.storageKey); if (raw) { const saved = readSavedRoom(raw); if (saved) return saved; } } catch { /* Storage may be unavailable in private browsing. */ }
...
function FloortrisWorkspace({ store: suppliedStore }: { store?: FloortrisStore }) {
  ...
  const [view, setView] = useState<View>(() => !['local','3m'].includes(session.sample) && store.getState().proposal ? 'proposal' : 'current');
  ...
  return <div className="ft-app ft-overhaul">
    <header className="ft-header"><Brand /><label className="ft-room-picker"><span>Rooms</span><select aria-label="Open a saved sample room" value={session.sample} onChange={e=>{const preset=ROOM_PRESETS.find(p=>p.id===e.target.value);if(preset)window.location.assign(preset.href);}}>{ROOM_PRESETS.map(p=><option key={p.id} value={p.id}>{p.label}</option>)}</select></label><h1>{state.room.name}</h1><div className="ft-header-actions"><span className="ft-save-label">Saved on this device</span><div className="ft-history" role="group" aria-label="Edit history"><button aria-label="Undo" title="Undo · ⌘/Ctrl Z (this session)" disabled={!store.getHistory().canUndo || showSetup || !!review} onClick={()=>travelHistory(false)}>↶</button><button aria-label="Redo" title="Redo · ⌘/Ctrl Shift Z (this session)" disabled={!store.getHistory().canRedo || showSetup || !!review} onClick={()=>travelHistory(true)}>↷</button></div><button className="ft-button ft-secondary" aria-expanded={panel === 'room'} onClick={() => togglePanel('room')}>Room <span aria-hidden="true">⌄</span></button></div></header>
    <main className="ft-stage">
      <section className="ft-canvas-panel" aria-label="Room planning workspace">
        ...
        <div className="ft-overlay-toolbar">{dimension === '3d' ? <span className="ft-3d-hint">Architectural preview · edit placement & inspect flags in 2D</span> : <div role="group" aria-label="Board overlay" className="ft-mode-tabs">{(['furniture','height','walk','tv','doors'] as OverlayMode[]).filter(m=>m!=='tv'||showTv).map(m => <button key={m} aria-pressed={mode === m} onClick={() => { setMode(m); setTvFlash(0); if(m==='tv')setTvBadge(false); }} className={(tvFlash?'tv':mode) === m ? 'active' : ''}>{m === 'tv' ? `TV${tvBadge?' !':''}` : m[0].toUpperCase() + m.slice(1)}</button>)}</div>}<div className="ft-health-flags">{(['path','tv','door'] as const).filter(f=>f!=='tv'||showTv).map(f => <button key={f} className={`ft-health-${flagState(f)}`} onClick={() => { setDimension('2d'); setMode(f === 'path' ? 'walk' : f === 'door' ? 'doors' : 'tv'); setPanel('check'); setSelected(null); }}><span aria-hidden="true">{flagState(f) === 'clear' ? '✓' : flagState(f) === 'missing' ? '○' : '!'}</span> {f === 'tv' ? 'TV' : f} · {flagState(f)}</button>)}</div></div>
      ...
      {(panel || item) && <aside className={`ft-drawer ${panel === 'pieces' ? 'ft-pieces-drawer' : ''}`} aria-label={panel ? `${panel} panel` : 'Selected piece'}><div className="ft-drawer-head"><h2>{panel === 'pieces' ? 'Pieces' : panel === 'room' ? 'Room' : panel === 'tools' ? 'Agent tools' : panel === 'check' ? `${view === 'compare' ? 'Yours' : which === 'proposal' ? 'Proposal' : 'Yours'} · room check` : item?.label}</h2><button className="ft-icon-button" aria-label="Close panel" onClick={() => { setPanel(null); setSelected(null); }}>×</button></div>
        ...
        {panel === 'room' && <><p className="ft-muted">Use Rooms above to switch samples. Each saves separately; your original room stays here.</p><button className="ft-button ft-secondary ft-full" onClick={()=>{setPanel(null);setSelected(null);setShowSetup(true);}}>Room inputs & fixed fixtures ↗</button><p className="ft-muted">{state.room.widthCm} × {state.room.depthCm} cm · edit your measurements and fixed features. Changes need your confirmation.</p>{(['wall','floor'] as const).map(target=><div key={target} className="ft-palette-section"><h3>{target === 'wall' ? 'Walls' : 'Floor'} · {which === 'current' ? 'Yours' : 'Proposal'}</h3><div className="ft-swatches">{PALETTES[target].map(p=><button key={p.id} disabled={!editable} title={p.name} aria-label={`Set ${p.name} ${target} finish`} aria-pressed={layout.appearance[target]===p.id} className={layout.appearance[target]===p.id?'active':''} style={{background:p.color}} onClick={()=>onResult(store.humanSetRoomFinish(which,target,p.id))}>{layout.appearance[target]===p.id?'✓':''}</button>)}</div></div>)}<button className="ft-button ft-secondary ft-full" onClick={exportRoom}>Export room JSON</button><button className="ft-text-button" onClick={()=>{if(window.confirm('Reset this room to its starting sample? Export first to keep a copy.')){store.resetDemo(session.makeInitial());changeView('current');}}}>Reset demo</button></>}
