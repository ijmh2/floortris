...
import { faces, type AppState, type Candidate, type CommandResult, type Furniture, type Layout, type Report, type Room, type Rules, type Wall, type Rotation } from './model.ts';
...
function FloortrisWorkspace({ store: suppliedStore }: { store?: FloortrisStore }) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      setNotice({ text: String(result.message || result.error?.message || 'History updated.'), error: !result.operationSucceeded });
      if (!store.getState().proposal) setView('current');
    };
    window.addEventListener('keydown', onKey); return () => window.removeEventListener('keydown', onKey);
  }, [store, showSetup, review]);
  ...
  return <div className="ft-app ft-overhaul">
    ...
    <main className="ft-stage">
      ...
      {(panel || item) && <aside className={`ft-drawer ${panel === 'pieces' ? 'ft-pieces-drawer' : ''}`} aria-label={panel ? `${panel} panel` : 'Selected piece'}><div className="ft-drawer-head"><h2>{panel === 'pieces' ? 'Pieces' : panel === 'room' ? 'Room' : panel === 'tools' ? 'Agent tools' : panel === 'check' ? `${view === 'compare' ? 'Yours' : which === 'proposal' ? 'Proposal' : 'Yours'} · room check` : item?.label}</h2><button className="ft-icon-button" aria-label="Close panel" onClick={() => { setPanel(null); setSelected(null); }}>×</button></div>
        ...
        {panel === 'room' && <><a className="ft-button ft-secondary ft-full" href={session.compact ? '/' : '/?sample=3m'}>{session.compact ? 'Return to original room ↗' : 'Open 3 × 3 m sample ↗'}</a>{session.compact && <p className="ft-muted">Separate sample · your original room is preserved. Assumed door, window and radiator positions. View the furnished proposal before applying.</p>}<button className="ft-button ft-secondary ft-full" onClick={()=>{setPanel(null);setSelected(null);setShowSetup(true);}}>Size, doors, windows & radiators ↗</button><p className="ft-muted">{state.room.widthCm} × {state.room.depthCm} cm · edit your measurements and fixed features. Changes need your confirmation.</p>{(['wall','floor'] as const).map(target=><div key={target} className="ft-palette-section"><h3>{target === 'wall' ? 'Walls' : 'Floor'} · {which === 'current' ? 'Yours' : 'Proposal'}</h3><div className="ft-swatches">{PALETTES[target].map(p=><button key={p.id} disabled={!editable} title={p.name} aria-label={`Set ${p.name} ${target} finish`} aria-pressed={layout.appearance[target]===p.id} className={layout.appearance[target]===p.id?'active':''} style={{background:p.color}} onClick={()=>onResult(store.humanSetRoomFinish(which,target,p.id))}>{layout.appearance[target]===p.id?'✓':''}</button>)}</div></div>)}<button className="ft-button ft-secondary ft-full" onClick={exportRoom}>Export room JSON</button><button className="ft-text-button" onClick={()=>{if(window.confirm('Reset this room to its starting sample? Export first to keep a copy.')){store.resetDemo(session.makeInitial());changeView('current');}}}>Reset demo</button></>}
