...
function FloortrisWorkspace({ store: suppliedStore }: { store?: FloortrisStore }) {
  ...
  useEffect(() => { try { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); } catch { queueMicrotask(() => setNotice({ text: 'Local saving unavailable. Export your room before closing.', error: true })); } }, [state]);
  ...
  return <div className="ft-app ft-overhaul">
    ...
    <main className="ft-stage">
      ...
      {(panel || item) && <aside className={`ft-drawer ${panel === 'pieces' ? 'ft-pieces-drawer' : ''}`} aria-label={panel ? `${panel} panel` : 'Selected piece'}><div className="ft-drawer-head"><h2>{panel === 'pieces' ? 'Pieces' : panel === 'room' ? 'Room' : panel === 'tools' ? 'Agent tools' : panel === 'check' ? `${view === 'compare' ? 'Yours' : which === 'proposal' ? 'Proposal' : 'Yours'} · room check` : item?.label}</h2><button className="ft-icon-button" aria-label="Close panel" onClick={() => { setPanel(null); setSelected(null); }}>×</button></div>
        ...
        {panel === 'room' && <><button className="ft-button ft-secondary ft-full" onClick={()=>{setPanel(null);setSelected(null);setShowSetup(true);}}>Size, doors & windows ↗</button><p className="ft-muted">{state.room.widthCm} × {state.room.depthCm} cm · demo measurements. Radiator is a fixed demo fixture.</p>{(['wall','floor'] as const).map(target=><div key={target} className="ft-palette-section"><h3>{target === 'wall' ? 'Walls' : 'Floor'} · {which === 'current' ? 'Yours' : 'Proposal'}</h3><div className="ft-swatches">{PALETTES[target].map(p=><button key={p.id} disabled={!editable} title={p.name} aria-label={`Set ${p.name} ${target} finish`} aria-pressed={layout.appearance[target]===p.id} className={layout.appearance[target]===p.id?'active':''} style={{background:p.color}} onClick={()=>onResult(store.humanSetRoomFinish(which,target,p.id))}>{layout.appearance[target]===p.id?'✓':''}</button>)}</div></div>)}<button className="ft-button ft-secondary ft-full" onClick={exportRoom}>Export room JSON</button><button className="ft-text-button" onClick={()=>{if(window.confirm('Reset to the demo? Export first to keep a copy.')){store.resetDemo();changeView('current');}}}>Reset demo</button></>}
