...
export type MeasurementSource = 'labelled' | 'provider' | 'human_confirmed' | 'inferred' | 'estimated';
export type MeasurementRecord = { target: string; source: MeasurementSource; confidence: number; note?: string };
export type MeasurementContext = { records: MeasurementRecord[]; assumptions: string[] };
export type AccommodationContext = { packId: string; providerId: string; buildingId: string; roomId: string; approvedVariantIds: string[]; fixedFurnitureIds: string[]; restrictions: string[] };
export type AccessibilityPlanningPack = { id: string; enabled: boolean; turningCircleCm: number; routeWidthCm: number; doorApproachDepthCm: number; bedTransferCm: number; deskApproachCm: number; reachableStorageMaxCm: number; maxProjectionCm: number };
export type Room = { name: string; widthCm: number; depthCm: number; floorPlan?: FloorPlan; openings: Opening[]; fixtures: Furniture[]; openingLocks?: string[]; profile?: RoomProfile; measurementContext?: MeasurementContext; accommodation?: AccommodationContext };
