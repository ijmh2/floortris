...
const geometry = object({ type: { enum: ['sectional'] }, primaryFacing: wall, modules: { type: 'array', items: moduleSchema, minItems: 2, maxItems: 12 } }, ['type','primaryFacing','modules']);
...
function itemError(item: Furniture, context: 'layout'|'inventory'|'fixture'): string | null {
  ...
  if (item.customProvenance || item.geometry) return `${item.id} forges custom provenance/geometry.`;
